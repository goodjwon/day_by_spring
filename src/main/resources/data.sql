...
INSERT INTO book (id, title, author, isbn, price, available, created_date) VALUES
                                                                               (1, 'Clean Code', 'Robert C. Martin', '978-0132350884', 45000, true, CURRENT_TIMESTAMP),
...
-- 회원 테이블의 자동 증가 시퀀스 재설정 (다음 ID는 5부터 시작)
ALTER TABLE member ALTER COLUMN id RESTART WITH 5;

-- 대출 데이터 (진행중)
INSERT INTO loan (id, member_id, book_id, loan_date, due_date) VALUES
                                                                   (1, 1, 1, DATEADD('DAY', -5, CURRENT_TIMESTAMP), DATEADD('DAY', 9, CURRENT_TIMESTAMP)),
...
INSERT INTO loan (id, member_id, book_id, loan_date, due_date, return_date) VALUES
                                                                                (4, 1, 4, DATEADD('DAY', -30, CURRENT_TIMESTAMP), DATEADD('DAY', -16, CURRENT_TIMESTAMP), DATEADD('DAY', -15, CURRENT_TIMESTAMP)),
                                                                                (5, 2, 5, DATEADD('DAY', -25, CURRENT_TIMESTAMP), DATEADD('DAY', -4, CURRENT_TIMESTAMP), DATEADD('DAY', -2, CURRENT_TIMESTAMP));

-- 대출 테이블의 자동 증가 시퀀스 재설정 (다음 ID는 6부터 시작)
